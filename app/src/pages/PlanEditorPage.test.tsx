...
describe('PlanEditorPage -- nyelv és pénznem (D21)', () => {
  ...
  it('shows the empty-currency message in the search when NOTHING matches and the plan currency has zero priced items', async () => {
    ...
    const search = await screen.findByPlaceholderText(/Tétel keresése/);
    // 62. tétel (D69) óta egy beárazatlan tétel is találat -- ez a jelzés
  ...
  it('62. tétel (D69): egy a terv pénznemében beárazatlan tétel is megtalálható és felvehető, "—" listaárral, 0-ás ajánlati árral', async () => {
    const user = userEvent.setup();
    seedWithNoEurPrices();
    render(<App />);
