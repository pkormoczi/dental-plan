...
| F5 (`pdf/` + `design/`) | `7c5e795` | 2026-09-05 | 29 találat (13 d-ref, 16 legacy-ref) 11 fájlban → 0. A legkisebb fázis, felderítő agent nélkül. Minden szám a tábláé és a témával egyezik, nincs elavult; 0 futásidejű string; 1 tesztnév (`backlog-9/…: előleg-sor` describe). A `TervDocument.tsx` fejléce PRODUCT.md § A nyomtatvány szerződéses dokumentum + `app/src/pdf/CLAUDE.md` pointert kapott a régi nyomtatvány-spec helyett; a `design/` `docs/07`-anchorai az `app/src/CLAUDE.md`-re mutatnak. Pdf+design suite zöld, teljes suite zöld, lint/build zöld. |
| F6 (`components/`) | `c0fbf07` | 2026-09-05 | 122 találat (96 d-ref, 26 legacy-ref) 32 fájlban → 0, plusz 3 kettőspontos `docs/07:` hivatkozás, amit egyik regex sem fogott. 75 címke-törlés (7 átfogalmazás, ahol a szám főnév/jelző volt: „…-védett felület" → „Mentés/Mégse-őrrel védett"), 3 WHY (címke vs. terv-mappa név; normál állapot jelvénye zaj; a piszkozat-őrök a globális draft felülírását védik), 13 anchor az `app/src/CLAUDE.md`-re / `domain/CLAUDE.md`-re, 15 levezethető anchor törölve, 1 describe-cím. Két-számozás: ebben a mappában három szám is ütközött fájlon belül (a nyelvi visszaesés vs. „egy aktív draft"; a metaadat-mező vs. „draftból szabadon kilépés"; a dirty-őr vs. a stepper-feliratok) — mind címke volt, a WHY helyben; a csere szövege a helyes forrás témáját követi. Két elavult `redesign DP-…` mutató törölve. 0 futásidejű string. Components-suite zöld, teljes suite zöld, lint/build zöld. |
| F7 (`pages/`) | `c03dcf2` | 2026-09-05 | 434 docs-check találat (351 d-ref, 83 legacy-ref) 68 fájlban → 0, plusz ~15 regexen kívüli hivatkozás (`docs/07` és `docs/03 §` kötőjel nélkül, `redesign DP-…` szöveg, 5 tesztnév-utótag). Gépi rész 149 (144 tiszta zárójeles címke, 5 `(Dn: szó)` → `(szó)`) — a scratchpad-script `[ \t]`-re szűkítve, sortörést nem nyel el (0 összevont sor); a forrásfájlok **CRLF**-esek, a többsoros cserélő ezt fájlonként kezeli. Kézi rész ~245 csere: ~90 címke/prefix-törlés és átfogalmazás, ahol a szám főnév volt („updater-szerződés óta", „Mentés/Mégse dirty-őr" / „NavBar-őr", „az árlista-tételek »csak deaktiválható« mintája", „betöltéskori dátumbélyegzés mintája"); 8 WHY-mondat (append-only verziómappa ×3, a kézi név újraírása törölné a doki pontosítását, a fix-összeg tárolás oka, a leírás nem esik magyarra ×2, a kedvezmény nem szerződéses tartalom); docs-anchorok: 9 PRODUCT.md (§ A nyomtatvány szerződéses dokumentum a placeholder-zárra és a kedvezményre, § Nem cél a Fog-jegyzetmezőre), 16 nested `CLAUDE.md` (`app/src` a11y/`≈`/billentyűzet/jsdom, `storage` updater + törzsadat, `domain` leírás-fallback + betöltéskori dátum, `pdf` placeholder-garancia), ~73 levezethető anchor törölve (a `docs/03 § 11` tervReszletei-fejlécek, § 6/7/8/9, Autosave, véglegesítés-lánc lépései) lokális mondattal; 14 tesztnév. Két-számozás a pages-ben: a quick-create, az „egy aktív draft" és a sortörlés-Undo alacsony számai a redesign-naplóé (a tábla azonos számai más témák) — mind címke volt, a szöveg a helyes témát mondja. 0 futásidejű string. Pages-suite 513 teszt zöld, teljes suite 1848 zöld, lint/build zöld. Lelet: `TervReszleteiPage.test.tsx` „75. tétel: PDF nélküli verzión…" tesztje időzítés-függő (a fejléc után szinkron kérdezi a még töltődő PDF-panel üzenetét) — egyszer piros, utána 3/3 zöld, a diff komment-only; külön tétel. |
| F8 (gyökér + maradék) | `774c3b3` | 2026-09-05 | 31 docs-check találat 5 gyökér-fájlban (`App.tsx`, `App.test.tsx`, `testUtils.tsx`, `testQueries.ts`, `index.css`: 30 d-ref, 5 legacy-ref) → 0, plusz 9 regexen kívüli hivatkozás 7 másik fájlban (kötőjel nélküli `docs/07` ×3 és `docs/02` ×2, két csupasz „redesign” említés, egy `backlog/plans/…` útvonal, a `fonts/README.md` 2 sora — ez utóbbi az F10 5. pontja, előrehozva). Mind kézi, script nélkül. Osztályozás: 20 címke/prefix-törlés (3 átfogalmazás, ahol a szám jelző volt: „…-védett felületek” → „Mentés/Mégse-őrrel védett”), 4 WHY-mondat (append-only ×3 az `App.test.tsx`-ben, „a lényeg, hogy egy német ajánlat is maradhat forintos”), 2 tesztnév-utótag, 9 levezethető anchor törölve (skeleton, Autosave, diff-tábla `—`, fa-rendezés), 3 anchor az `app/src/CLAUDE.md`-re (`controlBorder`) és az `app/src/pdf/CLAUDE.md`-re (font). Két-számozás a gyökérben: a quick-create és a lánc-rendezés száma a redesign-naplóé, a többi a tábláé — mind címke volt; egy szám (a „22 páciensre bővített demó-készlet” mellett) egyikre sem illett, törölve. `(K2)`/`P1-1`/`K4` review-címkék és a `backlog-N`/`N. tétel` említések az F3–F7 precedens szerint maradtak. 0 futásidejű string. Az `app/src` alatt a `docs-check` egyetlen találata a `dokumentacioGuard.test.ts` (a `docs/01`-et olvassa, F10 törli). Célzott 8 fájl 115 teszt zöld, teljes suite zöld, lint/build zöld. |
| F9 (skillek) | `398e691` | 2026-09-05 | `git mv browser-validation → manual-checks`; `git rm planning/SKILL.md`, `implement-backlog-item{,-worktree}.md`, `checklist.md`. Új: `plan/`, `implement/`, `finish/` SKILL.md (8,2 / 5,5 / 5,5 KB — a régi 15 + 6,9 + 9,3 KB helyett), `manual-checks/SKILL.md` átírva (11 KB) + `pdf.md`/`visual-css.md`/`keyboard-a11y.md` (4,3 / 6,5 / 6,4 KB, a régi 15 KB-os checklist szeletelve, a snippetek változatlanul). A `/plan` kimenete `backlog/plans/<slug>.md` (V2 §10 sablon, Baseline SHA, ≤ 6000 kar); az `/implement` a régi `backlog-N-*-terv.md` neveket is elfogadja, Baseline nélkül figyelmeztet; a `/finish` nem ír done-naplót, a tervfájlt `git rm`-eli, a `BACKLOG.md` szakaszát törli. **Doki döntése:** a következő tételszám forrása a `BACKLOG.md` fejlécének új `**Legutóbb kiosztott szám:** 118` sora (a lezárt tételek törlődnek a fájlból, a legacy napló nem forrás). A worktree-koreográfia a `/implement --worktree` (belépés, `npm install`) és a `/finish --worktree` (rebase, `--force-with-lease`, `gh pr create`) között osztva. Pontszerű: `doctor-review` 4 hely (`docs/07` → `app/src/CLAUDE.md`, `browser-validation` → `manual-checks`), `update-features` 2 hely (a „ne olvasd a régi spec-et” bekezdés törölve), `push-backlog-item` 1 hely. `docs-check`: 54 → 28 hiba (`.claude` 0; marad `docs/06` 17, `README.md` 10, `dokumentacioGuard.test.ts` 1 — mind F10). Guard-teszt zöld. Nyitott: a `/plan` skill-név ütközése a Claude Code beépített plan-módjával friss sessionben ellenőrizendő (a `/` skill-lista); ütközésnél a név `plan-item`, és a root `CLAUDE.md` Workflow-sora követi. |
| F10 | (a záró commit) | 2026-09-05 | `git mv` a `docs/legacy/` alá: a 01–05, 07 doksik, a leltár, a PROBLEMS, a V2-modell, és `backlog/done` → `docs/legacy/backlog-done/` (113 fájl + `redesign/`); `docs/legacy/README.md` a törlési dátummal (2026-11-04 után, külön commit). `git rm` a `dokumentacioGuard` hármas. `scripts/docs-check.mjs`: a `LEGACY_BOUND_DOCS` átmeneti kizárás törölve. `README.md` táblája újraírva (`PRODUCT.md`, `CLAUDE.md`, nested context, `docs/06`, backlog, seed, logó, `app/`), az MVP-határ és a nyitott kérdések a `PRODUCT.md`-re mutatnak; a legacy mappát útvonal nélkül említi. `docs/06`: 17 regexes + 8 kötőjel nélküli hivatkozás → `PRODUCT.md` szakasz (Adat- és deployment-korlátok ×5, Két fázis ×2, Szándékos hiányok ×1), `app/src/storage/CLAUDE.md` (mappastruktúra, törzsadat ×6), symbol-útvonal (`paciensTorles.ts`), a történeti „a korábbi technológia-leírás (ma legacy) azt állította” alak ×7; a `dokumentacioGuard`-buktató bekezdése a `docs-check` anchor-átírási szabályára cserélve. CI: `Docs check` lépés a tesztek után. A skillek átmeneti „hibaszám nem nőhet” mondata → „zöld (0 hiba)”. **Ellenőrzés (4. szakasz):** build/lint/teljes suite zöld; `docs-check` 0 hiba; D-grep az `app/src`-ben 0; legacy-grep 0; budget (Python kódpont-számlálás, a Git Bash `wc -m` bájtot számol): 3843/4000, 5753/6000, 2476–2497/2500; anchor-szabotázs (egy `symbol:` + egy `test:` elrontva) → 2 pontos `fájl:sor`, visszaállítva tiszta. `/run` izolált headless Chrome-ban, dev-szerver, CSP mellett: új páciens ő/ű névvel, tételfelvitel billentyűzettel (kereső ürül, fókusz marad, popover zárt), fogszám, előnézet, letöltés-instrumentálás (3 blob, mind localhost, külső URL 0), véglegesítés `_v1` — a PDF 3 oldal, `%PDF-1.3`, NotoSans Regular+SemiBold beágyazva (2 `FontFile2`, a fontfájlok 59 KB-tal letöltve), 8 kép-XObject (logó + fogtérkép-PNG a 16/26-tal), sávos `*` lábjegyzettel; **0 CSP-sértés az apptól**. Lelet 1: a `manual-checks/pdf.md` örökölt `fetch(blob:)` snippetje a CSP `connect-src` miatt tiltott — a `Blob`-elkapás `initScript`-tel váltja, a SKILL.md „Nem ellenőrizhető” táblája is javítva. Lelet 2 (nem a migrációé, backlogra): a `PreviewPage` `VeglegesitesChecklist` amber Callout-jában `<p>` a `<p>`-ben (Radix `Callout.Text` + `Text as="p"`) — React DOM-nesting hiba a konzolban; és a `Buffer is not defined` warn a react-pdf-től (ismert, ártalmatlan). **Falszifikációs teszt 6/6 helyes** friss subagenttel, legacy nélkül, 31 fájl a context-fájlokon felül; grep kellett a `setPlan`-hívók listájához, az `arlistaSzures` szűrő-helperhez és a `DemoStorage` `savingChain` sorosításához — az `arlistaSzures: tetelIlleszkedik` bekerült a `domain/CLAUDE.md` indexébe (két zárójel rövidítésével a budgeten belül), a többi helyesen megválaszolt, nem került be. A `/plan` skill friss sessionben betöltődik (a doki ellenőrizte), nem kell átnevezni. A próba-ciklus (`/plan → /implement → /finish`) és a CI-zöld a doki kézi feladata a push után. Ez a fájl ezután `docs/legacy/`-be kerül. |
