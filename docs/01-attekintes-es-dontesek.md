...
| D36 | A `/paciens`/`/terv`/`/elonezet` workflow-oldalakat közös héj veszi körül: állandó, kattintható breadcrumb (`Páciensek > [páciens neve]`, a páciens-szegmens egyelőre NEM link) + szabadon kattintható, route-vezérelt 3-lépéses stepper (`Terv adatai → Kezelések → Előnézet és véglegesítés`). A stepper a meglévő, laponkénti "Tovább" gombok MELLETT él, nem helyettük; ezzel a fő navigáció véglegesen ötelemű, a D34-ben átmenetileg megtartott négy workflow-link (`Páciens`/`Terv szerkesztő`/`Előnézet`/`Korábbi tervek`) megszűnt | A stepper formalizálja azt, ami korábban is igaz volt (a workflow-oldalak között szabadon lehetett navigálni) — a lépésjelző és az irányított "Tovább" CTA együtt élnek, nem egymást kizáró minta. A páciens-szegmens azért nem link, mert a draftra kötött stabil `patientDir`/`paciensId` ma nincs nyilvántartva (a leendő aktív draft lifecycle tétel hatóköre) — egy `buildPatientDirName()`-ből találgatott link nem garantáltan a valódi forrásmappára mutatna (`app/src/components/TervWorkflowShell.tsx`) |
